...
class LevelScene: SKScene, SKPhysicsContactDelegate {
    ...
    var player = Player()
    var enemy = Enemy(x: 5, y: 3, playerLevel: 0)
    ...
    override func didMoveToView(view: SKView) {
        if built {
            ...
            if fightLoser is Player {
                // A disappointing failure.
                self.removeAllActions()
                self.removeAllChildren()
                build()
            } else if fightLoser is Enemy {
                // A valiant victory! Congratulation!
                let enemy = fightLoser as! Enemy
                player.awardBonuses(enemy.weapons, enemyLevel: Int(floor(enemy.level)))
    ...
    func build() {
        ...
        player = Player()
        enemy = Enemy(x: 5, y: 3, playerLevel: player.level)
        map_size = 10
